...
| Data Quality Dashboard | `/quality` | M6 | M10 | Engineer+ (Admin, Data Engineer, BI Analyst) |
| Lineage Explorer | `/lineage` | M6 | M10 | Analyst+ (Admin, Data Engineer, BI Analyst) |
| Catalog Browser | `/catalog` | M7 | M10 | Viewer+ (all roles) |
| BI Report Builder | `/reports/builder` | M7 | M8 | Analyst only (Admin, BI Analyst) |
| Admin Panel | `/admin` | M8 | M11 | Admin only |
| User Management | `/admin/users` | M8 | M11 | Admin only |
...
| 15 | Filter Sidebar | M7 | Catalog Browser | None |
| 16 | Asset Card | M7 | Catalog Browser | Status Badge |
...
| Data Quality Dashboard | `GET /api/v1/quality/scores` | M10 — Data Quality Engine |
| Lineage Explorer | `GET /api/v1/lineage/{asset_id}` | M10 — Data Lineage |
| Catalog Browser | `GET /api/v1/catalog/search` | M10 — Data Catalog |
| BI Report Builder | `GET /api/v1/warehouse/query` | M9 — BI Dashboard Layer |
| Admin Panel | `GET /api/v1/auth/users`, `PUT /api/v1/auth/rbac` | M11 — Security |
| User Management | `GET /api/v1/auth/users`, `POST /api/v1/auth/users` | M11 — Security |
...
**Risk 3 — Catalog Browser Live Search**
